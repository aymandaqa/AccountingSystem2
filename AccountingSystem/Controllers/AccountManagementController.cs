    ...
    public class AccountManagementController : Controller
    {
        ...
        public async Task<IActionResult> SupplierBalances()
        {
            ...
            ViewBag.CompanyBranches = branchIds != null && branchIds.Any()
                ? await _context.CompanyBranches.Where(t => branchIds.Contains(t.Id)).ToListAsync()
                : await _context.CompanyBranches.ToListAsync();

            ViewBag.Suppliers = await _context.BusinessStatementBulk
        ...
        {
            var suppliersQuery = _context.BusinessStatementBulk.AsQueryable();
            ...
            var mappings = senderIds.Any()
                ? await _accontext.CusomerMappingAccounts
            ...
            var accounts = accountIds.Any()
                ? await _accontext.Accounts.Include(a => a.Branch).Where(a => accountIds.Contains(a.Id)).ToDictionaryAsync(a => a.Id)
                : new Dictionary<int, Account>();

            var branchIds = suppliers.Select(s => s.CompanyBranchID).Distinct().ToList();

            var branches = branchIds.Any()
                ? await _context.CompanyBranches.Where(b => branchIds.Contains(b.Id)).ToDictionaryAsync(b => b.Id)
        ...
        {
            var shipments = (from sh in _context.ShipmentSummary
